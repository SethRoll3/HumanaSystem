            ...
            >
              ...
              <div className="space-y-1">
                {dayApps.slice(0, 3).map(app => {
                  const resolvedName = getResolvedName(app);
                  return (
                    <div
                      key={app.id}
                      ...
                      title={`${format(app.date, 'HH:mm')} - ${resolvedName}`}
                    >
                      <div className="flex items-center gap-1">
                        ...
                        <span className="truncate">{format(app.date, 'HH:mm')} {resolvedName}</span>
                      </div>
                    </div>
                  );
                })}
...
export const AppointmentCalendar: React.FC<AppointmentCalendarProps> = ({ user }) => {
  ...
  const [rawAppointments, setRawAppointments] = useState<CalendarAppointment[]>([]);
  ...
  const filteredDayAppointments = dayAppointmentsModal
    ? dayAppointmentsModal.apps.filter(app => {
      ...
      if (!term) return true;
      const resolvedName = getPatientName(app);
  ...
  useEffect(() => {
    ...
    return () => unsubscribe(); // Limpiar suscripción al desmontar o cambiar la fecha de vista

  }, [viewDate, currentView]);

  // FILTRADO LOCAL
  useEffect(() => {
    let apps = [...rawAppointments];
  ...
  }, [rawAppointments, selectedDoctorId, selectedSpecialty, selectedStatus, patientSearchTerm, patientLookup, isDoctor, user?.uid, doctors]);
  ...
  */
  ...
  const handleConfirmPhone = async (id: string, method: string) => {
    ...
    try {
      await Promise.race([
        appointmentService.confirmByPhone(id, user?.uid || 'unknown', method),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout-Firebase')), 5000))
      ]);
      toast.success("Confirmada exitosamente", { id: loadingToast });
      setIsDetailsModalOpen(false);
      loadData();
    } catch (e: any) {
      console.error(e);
      const errMsg = e?.message || '';
      toast.error(errMsg.includes('Quota') || errMsg.includes('Timeout') ? "Error: Cuota de Firebase excedida o red lenta." : "Error al confirmar", { id: loadingToast });
    }
  };
  ...
  const handleCancelAppointment = async (id: string, reason: string) => {
    ...
    const loadingToast = toast.loading(reason === 'no_show_internal' ? "Marcando..." : "Cancelando...");
    try {
      await Promise.race([
        (async () => {
      ...
      toast.success(reason === 'no_show_internal' ? "Cita marcada como 'No se presentó'" : "Cancelada", { id: loadingToast });
      ...
      <div className="flex-1 h-[800px] overflow-hidden bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col">
        ...
        {currentView === 'month' ? (
          <CustomMonthView
            ...
            onSelectSlot={(d) => {
              if (!canCreate) return;
              if (d < new Date() && !isSameDay(d, new Date())) { toast.error("No pasado"); return; }
