            ...
            {canSeeNotifications && (
              <div className="relative">
                <button onClick={() => setShowNotifPanel(!showNotifPanel)} className="p-2.5 rounded-xl bg-slate-50 text-slate-500 hover:bg-brand-50 hover:text-brand-600 transition relative">
                  ...
                  {notifications.length > 0 && <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-red-500 text-white text-[10px] flex items-center justify-center rounded-full font-bold border-2 border-white animate-bounce">{notifications.length > 9 ? '9+' : notifications.length}</span>}
                </button>
                <AnimatePresence>
                  {showNotifPanel && (
                    <>
